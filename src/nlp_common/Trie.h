...
//---------------
template <class KEY, class DATA_TYPE>
Trie<KEY, DATA_TYPE>& Trie<KEY, DATA_TYPE>::operator=(const Trie<KEY, DATA_TYPE>& t)
{
  if (&t != this)
  {
    clear();
...
//--------------------------
template <class KEY, class DATA_TYPE>
const std::pair<std::vector<KEY>, DATA_TYPE>* Trie<KEY, DATA_TYPE>::const_iterator::operator->(void) const
{
  if (vecTriePtr.size() == 1 && vecTriePtr[0] == NULL)
  {
    return NULL;
