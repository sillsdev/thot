...
/**
 * @file SrcPosJumpFeat.cc
 *
 * @brief Definitions file for SrcPosJumpFeat.h
 */
...
template <>
SrcPosJumpFeat<PhrScoreInfo>::HypScoreInfo SrcPosJumpFeat<PhrScoreInfo>::extensionScore(
    ...
    const PhrHypDataStr& newHypDataStr, float weight, Score& unweightedScore)
{
  ...
  for (unsigned int i = predHypDataStr.sourceSegmentation.size(); i < newHypDataStr.sourceSegmentation.size(); ++i)
  {
    ...
    int prevSrcPosEnd;
    if (i > 0)
      prevSrcPosEnd = newHypDataStr.sourceSegmentation[i - 1].second;
    else
      prevSrcPosEnd = 0;
