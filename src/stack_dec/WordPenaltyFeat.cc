...
#include "stack_dec/WordPenaltyFeat.h"

//--------------- WordPenaltyFeat class functions

template <>
WordPenaltyFeat<PhrScoreInfo>::HypScoreInfo WordPenaltyFeat<PhrScoreInfo>::nullHypScore(
...
//---------------
template <>
WordPenaltyFeat<PhrScoreInfo>::HypScoreInfo WordPenaltyFeat<PhrScoreInfo>::extensionScore(
    ...
    const PhrHypDataStr& newHypDataStr, float weight, Score& unweightedScore)
{
  ...
  for (unsigned int i = predHypDataStr.sourceSegmentation.size(); i < newHypDataStr.sourceSegmentation.size(); ++i)
  {
    ...
    // Update score
    Score iterScore =
        wpModelPtr->sumWordPenaltyScore(trglen + nextTrgPhraseLen) - wpModelPtr->sumWordPenaltyScore(trglen);
