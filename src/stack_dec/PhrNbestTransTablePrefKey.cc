...
/**
 * @file PhrNbestTransTablePrefKey.cc
 *
 * @brief Definitions file for PhrNbestTransTablePrefKey.h
 */

//--------------- Include files --------------------------------------

#include "stack_dec/PhrNbestTransTablePrefKey.h"
...
bool PhrNbestTransTablePrefKey::operator<(const PhrNbestTransTablePrefKey& right) const
{
  ...
  if (right.ntrgSize < ntrgSize)
    return 1;
  if (lastGap < right.lastGap)
    return 0;
  if (right.lastGap < lastGap)
    return 1;
  return 0;
}
