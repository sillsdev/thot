...
LgProb Ibm2AlignmentModel::getIbm2AlignmentLgProb(const vector<WordIndex>& nsSent, const vector<WordIndex>& tSent,
                                            ...
                                            int verbose)
{
  ...
  if (verbose)
    cerr << "- lenLgProb(tlen=" << tSent.size() << " | slen=" << slen << ")= " << getSentenceLengthLgProb(slen, tlen)
         << endl;
