...
IncrHmmAligTrainer::IncrHmmAligTrainer(HmmAligModel& model, anjiMatrix& lanji, anjm1ip_anjiMatrix& lanjm1ip_anji)
    : lanji{lanji}, lanjm1ip_anji{lanjm1ip_anji}, model{model}
{
}

void IncrHmmAligTrainer::incrTrain(pair<unsigned int, unsigned int> sentPairRange, int verbosity)
{
  // EM algorithm
...
void IncrHmmAligTrainer::calcNewLocalSuffStats(pair<unsigned int, unsigned int> sentPairRange, int verbosity)
{
...
void IncrHmmAligTrainer::calcNewLocalSuffStatsVit(pair<unsigned int, unsigned int> sentPairRange, int verbosity)
{
...
void IncrHmmAligTrainer::calc_lanji(unsigned int n, const vector<WordIndex>& nsrcSent, const vector<WordIndex>& trgSent,
...
void IncrHmmAligTrainer::calc_lanji_vit(unsigned int n, const vector<WordIndex>& nsrcSent,
...
void IncrHmmAligTrainer::calc_lanjm1ip_anji(unsigned int n, const vector<WordIndex>& nsrcSent,
  ...
  for (unsigned int j = 1; j <= trgSent.size(); ++j)
  {
    ...
    for (unsigned int i = 1; i <= nsrcSent.size(); ++i)
    {
      ...
      if (j == 1)
      {
        ...
        bool nullAlig = model.isNullAlig(0, slen, i);
        double d;
        if (nullAlig)
        {
          if (model.isFirstNullAligPar(0, slen, i))
      ...
      else
      {
        for (unsigned int ip = 1; ip <= nsrcSent.size(); ++ip)
        {
          ...
          // Obtain information about alignment
          double d;
          bool validAlig = model.isValidAlig(ip, slen, i);
          if (!validAlig)
          {
            d = SMALL_LG_NUM;
          }
...
void IncrHmmAligTrainer::calc_lanjm1ip_anji_vit(unsigned int n, const vector<WordIndex>& nsrcSent,
...
void IncrHmmAligTrainer::gatherLexSuffStats(unsigned int mapped_n, unsigned int mapped_n_aux,
...
void IncrHmmAligTrainer::incrUpdateCountsLex(unsigned int mapped_n, unsigned int mapped_n_aux, PositionIndex i,
...
void IncrHmmAligTrainer::gatherAligSuffStats(unsigned int mapped_n, unsigned int mapped_n_aux,
                                             const vector<WordIndex>& nsrcSent, const vector<WordIndex>& trgSent,
...
void IncrHmmAligTrainer::incrUpdateCountsAlig(unsigned int mapped_n, unsigned int mapped_n_aux, PositionIndex slen,
...
void IncrHmmAligTrainer::incrMaximizeProbs()
{
...
float IncrHmmAligTrainer::obtainLogNewSuffStat(float lcurrSuffStat, float lLocalSuffStatCurr, float lLocalSuffStatNew)
{
